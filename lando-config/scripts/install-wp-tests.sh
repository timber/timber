...
# extract database creds for the database used for unit testing. The creds are defined in lando file
DB_HOST="database-unit-test"
DB_NAME="$(echo $LANDO_INFO | jq -r '.[] | {service, external_connection, creds} | select(.service == "database-unit-test") | .creds | .database')"
DB_USER="$(echo $LANDO_INFO | jq -r '.[] | {service, external_connection, creds} | select(.service == "database-unit-test") | .creds | .user')"
