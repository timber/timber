...
class Pagination
{
    ...
    protected function init($prefs = [], $wp_query = null)
    {
        ...
        // Take next and prev from a window of one page around the current page, because the
        // neighbours in $this->pages are dots when mid_size is 0.
        $window = self::paginate_links(\array_merge($args, [
            'show_all' => false,
            'start_size' => 0,
            'end_size' => 0,
            'mid_size' => 1,
        ]));
        foreach ($window as $key => $page) {
        ...
        if (isset($current) && isset($window[$current + 1]['link'])) {
            $this->next = [
                'link' => $window[$current + 1]['link'],
