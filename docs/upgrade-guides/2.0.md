...
- `excerpt({
    words: 50               // number of words in the excerpt
    chars: false            // number of characters in the excerpt. Default: false (no character limit)
    end: "&hellip;"         // string to append to the end of the excerpt. Default: "&hellip;" (HTML ellipsis character)
    force: false            // Whether to shorten the excerpt to the length/word count specified, if the editor wrote a manual excerpt longer than the set length
    strip: true             // Strip HTML tags?
    read_more: "Read More"  // String for what the "Read More" text should be.
    })` - This finally cleans up how the previews of posts are handled
