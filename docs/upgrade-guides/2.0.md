...
This property is now **protected** and you can **no longer access meta values through it**. It only acts as a reference for you to see which meta values exist for an object when you use `{{ dump() }}` or `var_dump()`. To access the values, you should always use the `meta()` and `raw_meta()` methods.
