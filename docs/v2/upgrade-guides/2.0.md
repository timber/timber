...
Version 2.0 of Timber removes a lot of deprecated code and tries to make the naming of functions and filters more consistent. We refactored how Timber Core works under the hood to improve compatibility with WordPress Core and be ready for future challenges.
...
Using the same classes as functions in Twig is also deprecated. There are new functions to get Timber objects in Twig:

- `{{ TimberPost() }}` – use `{{ get_post() }}` instead
- `{{ TimberImage() }}` – use `{{ get_image() }}` or `{{ get_attachment() }}` instead
- `{{ TimberTerm() }}` – use `{{ get_term() }}` instead
- `{{ TimberUser() }}` – use `{{ get_user() }}` instead
