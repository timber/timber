...
class TwigTest extends TimberIntegrationTestCase
{
    ...
    {
        \load_textdomain('timber-test', __DIR__ . '/Fixtures/languages/timber-test-en_US.mo');
    ...
    public function testWordPressPasswordFilters()
    {
        ...
        $context = [];
        \add_filter('protected_title_format', fn ($title) => 'Protected: ' . $title);
        $context['post'] = Timber::get_post($post_id);
        if (\post_password_required($post_id)) {
            $this->assertTrue(true);
            $str = Timber::compile('assets/test-wp-filters.twig', $context);
            $this->assertEquals('Protected: My Private Post', \trim($str));
        } else {
    ...
    public function testTwigString()
    {
        ...
        $this->assertEquals('FooxBarxQuack', \trim(Timber::compile_string($twig, [
            'string' => $str,
            'array' => $arr,
        ])));
    }
    ...
    public function testSetSimple()
    {
        ...
        $this->assertEquals('jiggy', \trim($result));
    }
    ...
    public function testSetObject()
    {
        ...
        $this->assertEquals('Spaceballs: may the schwartz be with you', \trim($result));
    }
    ...
    public function testAddToTwig()
    {
        \add_filter('timber/twig', function ($twig) {
            $twig->addFilter(new TwigFilter('foobar', fn ($text) => $text . 'foobar'));
            return $twig;
        });
    ...
    {
        \add_filter('timber/twig', function ($twig) {
            $twig->addFilter(new TwigFilter('quack', fn ($text) => $text . ' Quack!'));
            return $twig;
        });
    ...
    {
        \add_shortcode('my_shortcode', fn ($atts, $content) => 'Jaredfoo');
    ...
    {
        \add_shortcode('duck', fn ($atts, $content) => $content . ' says quack!');
