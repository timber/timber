...
class CustomMenuItemClass extends MenuItem
{
}
...
class MenuTest extends TimberIntegrationTestCase
{
    ...
    public function testNavMenuThumbnailsWithInitializedMenu()
    {
        ...
        // File may have suffix due to collision (e.g., arch-2.jpg)
        $pattern = '#^http://example\.org/wp-content/uploads/\d{4}/\d{2}/arch(-\d+)?\.jpg$#';
        $this->assertMatchesRegularExpression($pattern, $post->thumbnail());
        ...
        $expected_pattern = '#^' . \preg_quote((string) $menu_items[0]->ID, '#') . ' - http://example\.org/wp-content/uploads/\d{4}/\d{2}/arch(-\d+)?\.jpg$#';
    ...
    public function testCustomArchivePage()
    {
        $this->add_filter_temporarily('nav_menu_css_class', function ($classes, $menu_item) {
            if (\trailingslashit($menu_item->url) == \trailingslashit('http://example.org/gallery')) {
    ...
    {
        $pg_1 = static::factory()->post->create([
            'post_type' => 'page',
            'post_title' => 'Foo Page',
            'menu_order' => 10,
        ]);
        $pg_2 = static::factory()->post->create([
            'post_type' => 'page',
            'post_title' => 'Bar Page',
            'menu_order' => 1,
        ]);
    ...
    public function testMenuWalker()
    {
        ...
        // Remove this filter that prevents duplicate IDs when rendering menu multiple times.
        // Must be removed BEFORE both renders for consistent output.
