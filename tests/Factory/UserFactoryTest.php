<?php

namespace Timber\Tests\Factory;

use InvalidArgumentException;
use PHPUnit\Framework\Attributes\Group;
use Timber\Factory\UserFactory;
use Timber\Tests\TimberIntegrationTestCase;
use Timber\Timber;
use Timber\User;
use WP_User;
use WP_User_Query;

class AdminUser extends User
{
}
class SpecialUser extends User
{
}

class BadUser
{
}

#[Group('factory')]
#[Group('users-api')]
class UserFactoryTest extends TimberIntegrationTestCase
{
    public function testGetUser()
    {
        $id = static::factory()->user->create([
            'user_email' => 'me@example.com',
        ]);

        $userFactory = new UserFactory();
        $user = $userFactory->from($id);

        $this->assertInstanceOf(User::class, $user);
    }

    public function testGetUserFromInvalidId()
    {
        $userFactory = new UserFactory();
        $user = $userFactory->from(3345);

        $this->assertNull($user);
    }

    public function testGetUserFromIdString()
    {
        $user_id = static::factory()->user->create([
            'user_email' => 'me@example.com',
        ]);

        $userFactory = new UserFactory();
        $user = $userFactory->from('' . $user_id);

        $this->assertInstanceOf(User::class, $user);
        $this->assertEquals($user_id, $user->id);
    }

    public function testGetUserFromEmptyArray()
    {
        $userFactory = new UserFactory();
        $res = $userFactory->from([]);

        $this->assertEquals([], $res);
    }

    public function testGetUserWithOverrides()
    {
        $my_class_map = (fn (string $class, WP_User $user) => \in_array('administrator', $user->roles)
            ? AdminUser::class
            : User::class);
        \add_filter('timber/user/class', $my_class_map, 10, 2);

        $admin_id = static::factory()->user->create([
            'user_email' => 'me@example.com',
            'role' => 'administrator',
        ]);
        $normie_id = static::factory()->user->create([
            'user_email' => 'someone@example.com',
        ]);

        $userFactory = new UserFactory();
        $admin = $userFactory->from($admin_id);
        $normie = $userFactory->from($normie_id);

        $this->assertTrue(AdminUser::class === $admin::class);
        $this->assertTrue(User::class === $normie::class);

        \remove_filter('timber/user/class', $my_class_map);
    }

    public function testGetUserWithArrayOfIds()
    {
        $my_class_map = (fn (string $class, WP_User $user) => \in_array('administrator', $user->roles)
            ? AdminUser::class
            : User::class);
        \add_filter('timber/user/class', $my_class_map, 10, 2);

        $admin_id = static::factory()->user->create([
            'user_email' => 'me@example.com',
            'role' => 'administrator',
        ]);
        $normie_id = static::factory()->user->create([
            'user_email' => 'someone@example.com',
        ]);

        $userFactory = new UserFactory();

        // pass a list of IDs
        [$admin, $normie] = $userFactory->from([$admin_id, $normie_id]);

        $this->assertTrue(AdminUser::class === $admin::class);
        $this->assertTrue(User::class === $normie::class);

        \remove_filter('timber/user/class', $my_class_map);
    }

    public function testGetUserWithArrayOfIdsIncludingInvalidIds()
    {
        $user_id = static::factory()->user->create();
        $userFactory = new UserFactory();

        // Pass a list of IDs.
        [$user, $invalid] = $userFactory->from([$user_id, 256]);

        $this->assertInstanceOf(User::class, $user);
        $this->assertNull($invalid);
    }

    public function testGetNonExistentUser()
    {
        $user_id = static::factory()->user->create();
        $maybe_user = Timber::get_user_by('id', 3424);
        $this->assertNull($maybe_user);
    }

    public function testInvalidUserClassThrowsError()
    {
        $this->expectException(InvalidArgumentException::class);

        $bad_user_obj = new BadUser();
        $userFactory = new UserFactory();

        $normie_id = static::factory()->user->create([
            'user_email' => 'someone@example.com',
        ]);

        $normie_wp_user = Timber::get_user($bad_user_obj);
        $this->assertInstanceOf(User::class, $userFactory->from($normie_wp_user));
    }

    public function testGetUserFromWpUserObject()
    {
        $my_class_map = (fn (string $class, WP_User $user) => \in_array('administrator', $user->roles)
            ? AdminUser::class
            : User::class);
        \add_filter('timber/user/class', $my_class_map, 10, 2);

        $admin_id = static::factory()->user->create([
            'user_email' => 'me@example.com',
            'role' => 'administrator',
        ]);
        $normie_id = static::factory()->user->create([
            'user_email' => 'someone@example.com',
        ]);

        $admin_wp_user = \get_user_by('id', $admin_id);
        $normie_wp_user = \get_user_by('id', $normie_id);

        $userFactory = new UserFactory();
        $this->assertTrue(AdminUser::class === $userFactory->from($admin_wp_user)::class);
        $this->assertTrue(User::class === $userFactory->from($normie_wp_user)::class);

        \remove_filter('timber/user/class', $my_class_map);
    }

    public function testGetUserWithAssortedArray()
    {
        $my_class_map = (fn (string $class, WP_User $user) => \in_array('administrator', $user->roles)
            ? AdminUser::class
            : User::class);
        \add_filter('timber/user/class', $my_class_map, 10, 2);

        $admin_id = static::factory()->user->create([
            'user_email' => 'me@example.com',
            'role' => 'administrator',
        ]);
        $normie_id = static::factory()->user->create([
            'user_email' => 'someone@example.com',
        ]);
        $editor_id = static::factory()->user->create([
            'user_login' => 'ddd',
            'user_email' => 'ed@example.com',
            'role' => 'editor',
        ]);

        // create instances of different kinds of user objects
        $userFactory = new UserFactory();
        $admin_user = $userFactory->from($admin_id);
        $normie_wp_user = \get_user_by('id', $normie_id);

        // pass an array containing a User, a WP_User instance, and an ID
        $users = $userFactory->from([$admin_user, $normie_wp_user, $editor_id]);

        $this->assertTrue(AdminUser::class === $users[0]::class);
        $this->assertTrue(User::class === $users[1]::class);
        $this->assertTrue(User::class === $users[2]::class);

        \remove_filter('timber/user/class', $my_class_map);
    }

    public function testGetUserWithQueryArray()
    {
        $my_class_map = (fn (string $class, WP_User $user) => \in_array('administrator', $user->roles)
            ? AdminUser::class
            : User::class);
        \add_filter('timber/user/class', $my_class_map, 10, 2);

        $subscriber_id = static::factory()->user->create([
            'user_login' => 'aaa',
            'user_email' => 'sub@example.com',
            'role' => 'subscriber',
        ]);
        $admin_id = static::factory()->user->create([
            'user_login' => 'bbb',
            'user_email' => 'administrator@example.com',
            'role' => 'administrator',
        ]);
        $author_id = static::factory()->user->create([
            'user_login' => 'ccc',
            'user_email' => 'author@example.com',
            'role' => 'author',
        ]);

        // create instances of different kinds of user objects
        $userFactory = new UserFactory();

        // pass an array containing a User and WP_User instance
        // Use include to only get the users we created (exclude default admin user)
        $users = $userFactory->from([
            'role__in' => ['administrator', 'author'],
            'include' => [$admin_id, $author_id],
        ]);

        $this->assertCount(2, $users);
        $this->assertTrue(AdminUser::class === $users[0]::class);
        $this->assertTrue(User::class === $users[1]::class);

        \remove_filter('timber/user/class', $my_class_map);
    }

    public function testGetUserWithUserQuery()
    {
        $my_class_map = (fn (string $class, WP_User $user) => \in_array('administrator', $user->roles)
            ? AdminUser::class
            : User::class);
        \add_filter('timber/user/class', $my_class_map, 10, 2);

        $subscriber_id = static::factory()->user->create([
            'user_login' => 'aaa',
            'user_email' => 'sub@example.com',
        ]);
        $admin_id = static::factory()->user->create([
            'user_login' => 'bbb',
            'user_email' => 'admin@example.com',
            'role' => 'administrator',
        ]);
        $author_id = static::factory()->user->create([
            'user_login' => 'ccc',
            'user_email' => 'author@example.com',
            'role' => 'author',
        ]);

        // create instances of different kinds of user objects
        $userFactory = new UserFactory();
        // Use include to only get the users we created (exclude default admin user)
        $userQuery = new WP_User_Query([
            'role__in' => ['administrator', 'author'],
            'include' => [$admin_id, $author_id],
        ]);

        // pass an array containing a User and WP_User instance
        $users = $userFactory->from($userQuery);

        $this->assertCount(2, $users);
        $this->assertTrue(AdminUser::class === $users[0]::class);
        $this->assertTrue(User::class === $users[1]::class);

        \remove_filter('timber/user/class', $my_class_map);
    }
}
