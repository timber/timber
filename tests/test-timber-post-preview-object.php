<?php
	...
	class TestTimberPostPreviewObject extends Timber_UnitTestCase {
		...
		function test1886Error() {
			...
			$post_id = self::factory()->post->create(array('post_content' => $expected.'<blockquote>Lincoln</blockquote>', 'post_excerpt' => false));
		...
		function test1886ErrorWithForce() {
			$expected = '<p>Govenment:</p> <ul> <li>of the <strong>people</strong></li> <li>by the people</li> <li>for the people</li> </ul>';
			$post_id = self::factory()->post->create(array('post_excerpt' => $expected, 'post_content' => $this->gettysburg));
			$post = new Timber\Post($post_id);
		...
		function testPostPreviewObjectWithCharAndWordLengthWordsWin() {
			$pid = self::factory()->post->create( array('post_content' => $this->gettysburg, 'post_excerpt' => '') );
		...
		function testPostPreviewObjectWithLength() {
			$pid = self::factory()->post->create( array('post_content' => 'Lauren is a duck she a big ole duck!', 'post_excerpt' => '') );
		...
		function testPostPreviewObjectWithForcedLength() {
			$pid = self::factory()->post->create( array('post_content' => 'Great Gatsby', 'post_excerpt' => 'In my younger and more vulnerable years my father gave me some advice that I’ve been turning over in my mind ever since.') );
		...
		function testPostPreviewObject() {
			$pid = self::factory()->post->create( array('post_content' => 'Great Gatsby', 'post_excerpt' => 'In my younger and more vulnerable years my father gave me some advice that I’ve been <a href="http://google.com">turning over</a> in my mind ever since.') );
		...
		function testPreviewWithStripAndClosingPTag() {
			$pid = self::factory()->post->create( array('post_excerpt' => '<p>Lauren is a duck, but a great duck let me tell you why</p>') );
			$post = new TimberPost( $pid );
			$template = '{{post.preview.strip(false)}}';
			$str = Timber::compile_string($template, array('post' => $post));
		...
		function testEmptyPreview() {
			$pid = self::factory()->post->create( array('post_excerpt' => '', 'post_content' => '') );
			$post = new TimberPost( $pid );
			$template = '{{ post.preview }}';
			$str = Timber::compile_string($template, array('post' => $post));
		...
		function testPagePreviewOnSearch() {
			$pid = self::factory()->post->create(array('post_type' => 'page', 'post_content' => 'What a beautiful day for a ballgame!', 'post_excerpt' => ''));
			$post = new TimberPost( $pid );
			$template = '{{ post.preview }}';
			$str = Timber::compile_string($template, array('post' => $post));
