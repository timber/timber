...
class Twig {
	...
	public function add_timber_functions( $twig ) {
		...
		$twig->addFunction(new TwigFunction('Post', function( $pid, $PostClass = '' ) {
		...
		$twig->addFunction(new TwigFunction('Image', function( $pid, $ImageClass = 'Timber\Image' ) {
			if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
				foreach ( $pid as &$p ) {
					$p = new $ImageClass($p);
				}
		...
		$twig->addFunction(new TwigFunction('Term', function( $pid, $TermClass = '' ) {
			if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
				foreach ( $pid as &$p ) {
		...
		$twig->addFunction(new TwigFunction('User', function( $pid, $UserClass = 'Timber\User' ) {
			if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
				foreach ( $pid as &$p ) {
					$p = new $UserClass($p);
				}
