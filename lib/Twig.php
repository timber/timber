...
class Twig {
	...
	public function add_timber_functions( $twig ) {
		/* actions and filters */
		$twig->addFunction(new Timber_Function('action', function( $context ) {
					$args = func_get_args();
					array_shift($args);
					$args[] = $context;
		...
		$twig->addFunction(new Timber_Function('function', array(&$this, 'exec_function')));
		$twig->addFunction(new Timber_Function('fn', array(&$this, 'exec_function')));

		$twig->addFunction(new Timber_Function('shortcode', 'do_shortcode'));

		/* TimberObjects */
		$twig->addFunction(new Timber_Function('TimberPost', function( $pid, $PostClass = 'Timber\Post' ) {
		...
		$twig->addFunction(new Timber_Function('TimberImage', function( $pid = false, $ImageClass = 'Timber\Image' ) {
					if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
						foreach ( $pid as &$p ) {
							$p = new $ImageClass($p);
						}
		...
		$twig->addFunction(new Timber_Function('TimberTerm', function( $pid, $TermClass = 'Timber\Term' ) {
					if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
						foreach ( $pid as &$p ) {
							$p = new $TermClass($p);
						}
		...
		$twig->addFunction(new Timber_Function('TimberUser', function( $pid, $UserClass = 'Timber\User' ) {
					if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
						foreach ( $pid as &$p ) {
							$p = new $UserClass($p);
						}
		...
		/* TimberObjects Alias */
		$twig->addFunction(new Timber_Function('Post', function( $pid, $PostClass = 'Timber\Post' ) {
					if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
						foreach ( $pid as &$p ) {
							$p = new $PostClass($p);
						}
		...
		$twig->addFunction(new Timber_Function('Image', function( $pid, $ImageClass = 'Timber\Image' ) {
					if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
						foreach ( $pid as &$p ) {
							$p = new $ImageClass($p);
						}
		...
		$twig->addFunction(new Timber_Function('Term', function( $pid, $TermClass = 'Timber\Term' ) {
					if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
						foreach ( $pid as &$p ) {
							$p = new $TermClass($p);
						}
		...
		$twig->addFunction(new Timber_Function('User', function( $pid, $UserClass = 'Timber\User' ) {
					if ( is_array($pid) && !Helper::is_array_assoc($pid) ) {
						foreach ( $pid as &$p ) {
							$p = new $UserClass($p);
						}
		...
		$twig->addFunction(new Timber_Function('bloginfo', function( $show = '', $filter = 'raw' ) {
					return get_bloginfo($show, $filter);
				} ));
		$twig->addFunction(new Timber_Function('__', function( $text, $domain = 'default' ) {
					return __($text, $domain);
				} ));
		$twig->addFunction(new Timber_Function('translate', function( $text, $domain = 'default' ) {
					return translate($text, $domain);
				} ));
		$twig->addFunction(new Timber_Function('_e', function( $text, $domain = 'default' ) {
					return _e($text, $domain);
				} ));
		$twig->addFunction(new Timber_Function('_n', function( $single, $plural, $number, $domain = 'default' ) {
					return _n($single, $plural, $number, $domain);
				} ));
		$twig->addFunction(new Timber_Function('_x', function( $text, $context, $domain = 'default' ) {
					return _x($text, $context, $domain);
				} ));
		$twig->addFunction(new Timber_Function('_ex', function( $text, $context, $domain = 'default' ) {
					return _ex($text, $context, $domain);
				} ));
		$twig->addFunction(new Timber_Function('_nx', function( $single, $plural, $number, $context, $domain = 'default' ) {
					return _nx($single, $plural, $number, $context, $domain);
				} ));
		$twig->addFunction(new Timber_Function('_n_noop', function( $singular, $plural, $domain = 'default' ) {
					return _n_noop($singular, $plural, $domain);
				} ));
		$twig->addFunction(new Timber_Function('_nx_noop', function( $singular, $plural, $context, $domain = 'default' ) {
					return _nx_noop($singular, $plural, $context, $domain);
				} ));
		$twig->addFunction(new Timber_Function('translate_nooped_plural', function( $nooped_plural, $count, $domain = 'default' ) {
